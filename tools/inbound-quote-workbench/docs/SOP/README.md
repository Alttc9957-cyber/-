...
从 2026-07-06 起，每轮开工前还必须读取：

1. `docs/PRODUCT_POSITIONING.md`
2. `docs/STAGE_PLAN.md`
3. `docs/CODEX_EXECUTION_LOOP.md`
4. 当前 `goals/<module>/GOAL.md` 和 `goals/<module>/ACCEPTANCE.md`，如存在
...
客户需求 -> 智能线路推荐 -> 产品库匹配 -> 供应商成本参考 -> 智能报价 -> 报价表下载 -> 成交转订单 -> OP 执行 -> 老板经营看板。
...
当前阶段内，优先级是 A2 产品库真实数据修复与成本可信化。产品库成本未稳定前，不推进订单大功能；报价系统未稳定前，不推进老板看板。

### Codex 自循环要求

Codex 是唯一 Coder。用户只做阶段级验收。
